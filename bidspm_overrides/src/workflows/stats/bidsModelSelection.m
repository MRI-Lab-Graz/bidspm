...
function checks(opt)
  ...
  % container v4.0.0 bug: this used to unconditionally redo the cellfun call
  % above without 'UniformOutput', false (the `if iscell(space)` guard is
  % always true right after the line above, so it always ran) which crashes
  % with "cellfun: all values must be scalars when UniformOutput = true"
