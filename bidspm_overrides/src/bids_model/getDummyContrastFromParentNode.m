function dummyContrastsList = getDummyContrastFromParentNode(model, node)
  ...
  % Note: this assumes the root is a Run level node. A Run node has no
  % "parent" to inherit from, so fall back to its own auto-derived dummy
  % contrasts (its HRF Variables) -- mirrors the 'case run' branch in
  % getDummyContrastsList.m, which is what runs when this function isn't
  % reached via the parent-node recursion.
