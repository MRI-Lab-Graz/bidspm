...
function condToModel = addCondition(opt, condName, trialTypes, tsv, condToModel, varToConvolve)
  ...
  % Boolean indicator column (e.g. from the Factor transformer): condName is
  % empty because the variable has no dot-separator. Use the indicator directly
  % and treat the full column name (varToConvolve) as the condition name.
  if isempty(condName) && (islogical(trialTypes) || isnumeric(trialTypes))
    condName = varToConvolve;
    rows = find(trialTypes);
  else
