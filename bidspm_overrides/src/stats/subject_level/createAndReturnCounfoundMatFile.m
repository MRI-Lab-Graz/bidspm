function [counfoundFile, counfoundMeta] = createAndReturnCounfoundMatFile(opt, tsvFile)
  ...
  % Only apply transformers whose Input columns are present in the confounds
  % TSV. Run-level event transformers (e.g. MergeIdenticalRows) reference
  % columns like trial_type that do not exist here and would crash on the
  % missing 'onset' field.
  if ~isempty(transformers)
    confoundFields = fieldnames(content);
    validTransformers = {};
    for i_t = 1:numel(transformers)
      % Octave decodes heterogeneous JSON arrays as cell arrays of structs.
      ...
      if isfield(t, 'Input')
        if iscell(t.Input)
          inputs = t.Input;
        elseif ischar(t.Input)
          inputs = {t.Input};
        else
          inputs = {};
        end
        if ~any(ismember(inputs, confoundFields))
          continue
        end
      end
      validTransformers{end + 1} = t; %#ok<AGROW>
    end
    if ~isempty(validTransformers)
      content = bids.transformers(validTransformers, content);
    end
  end
