function copyAtlasToSpmDir(varargin)
  ...
  % container v4.0.0 bug: this used to always check
  % exist(targetAtlasImage(1:end - 3), 'file'), assuming targetAtlasImage
  % always ends in '.gz' (true only for 'aal', whose source is
  % AAL3v1_1mm.nii.gz). For 'hcpex'/'glasser'/'visfatlas'/'wang',
  % prepareFiles() already returns a plain .nii path, so stripping the last
  % 3 characters chops "nii" instead of ".gz" and checks for a file that can
  % never exist (e.g. "..._dseg." with a trailing dot) -- atlasPresent was
  % therefore always false for those 4 atlases, forcing every single
  % invocation to unconditionally re-copy (and, for 'wang', re-merge and
  % delete its source) regardless of whether the target already existed.
  % Confirmed live: this is what caused concurrent stats-workers to race on
  % the shared atlas cache/source dirs (crashes like "copyfile: no files to
  % move" and "delete: no such file"). Check the real target path, stripping
  % '.gz' only when actually present.
  if numel(targetAtlasImage) > 3 && strcmpi(targetAtlasImage(end - 2:end), '.gz')
    targetAtlasImageToCheck = targetAtlasImage(1:end - 3);
  else
    targetAtlasImageToCheck = targetAtlasImage;
  end

  atlasPresent = isdir(spmAtlasDir) && ...
                 exist(targetAtlasImageToCheck, 'file') && ...
