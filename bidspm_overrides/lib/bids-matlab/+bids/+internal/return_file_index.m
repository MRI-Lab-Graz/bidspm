function file_idx = return_file_index(BIDS, modality, filename)
  ...
  try
    ...
    msg = sprintf(['An error occurred when processing', ...
                  ...
                  filename);
    tolerant = true;
    % fMRIPrep writes HTML/SVG QC files into a 'figures' modality directory;
    % these are not BIDS neuroimaging files so indexing failures are always benign.
    verbose = ~strcmp(modality, 'figures');
