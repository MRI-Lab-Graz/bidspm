...
def run_local_bidspm_cli(config: Config, action: str, subjects: List[str], task: str, model_file_path: Path):
    ...
    for subject in subjects:
        ...
        try:
            ...
            env["BIDSPM_SKIP_OCTAVE_FORGE"] = "1"
            env["BIDSPM_SKIP_MACS_UPDATE"] = "1"
            env.setdefault("SPM_HTML_BROWSER", "0")
            ...
            try:
                for line in process.stdout:
                    print(line, end='')  # Stream to web interface
            ...
            if result.returncode == 0:
                ...
                # We log to file, but we don't need to print to stdout again (log function prints to stdout)
                # So we manually write to log file to avoid duplication in terminal
                log_path = Path(LOG_FILE)
                if hasattr(os, 'makedirs'): # Python < 3.2
                   if not os.path.exists(os.path.dirname(str(log_path))):
                       os.makedirs(os.path.dirname(str(log_path)))
                else: 
                   log_path.parent.mkdir(parents=True, exist_ok=True)
                   
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                with open(log_path, "a") as f:
                    f.write(f"{timestamp} [DEBUG] STDOUT: {result.stdout}\n")
            else:
                print(f"❌ {action.title()} failed for subject {subject}")
                # Log error output to file as well
                log_path = Path(LOG_FILE)
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                with open(log_path, "a") as f:
                    f.write(f"{timestamp} [ERROR] STDOUT/STDERR: {result.stdout}\n")
